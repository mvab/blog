...
The function takes `tophits_list`, `exposure_dat`, `outcome.id.mrbase` (dataset id in MR-Base)- or, if you want to use local data for the outcome, use `outcome.data` argument (expects full GWAS data in `.outcome` format). It then returns `beta` and `se` values for each exposure in `mvmr_input$XGs` and same for the outcome in `mvmr_input$Y`.
...
```{r, echo=F}
result_2smr<- result_2smr %>% select(-nsnp)
```
...
```{r, echo=F , fig.height = 3, fig.width=6, fig.retina=2, out.width="85%"}
library(ggplot2)
library(cowplot)
