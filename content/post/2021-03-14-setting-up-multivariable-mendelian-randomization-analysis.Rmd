...
**For the analysis we need:**
 ...
 - outcome GWAS full summary stats (will be accessed with TwoSampleMR from MR-Base)
 
```{r, echo=F}
data_path<-"../data/mvmr_tutorial/"
source(paste0(data_path, "functions_copy.R"))
# to run: rmarkdown::render("content/post/draft_blogpost.Rmd")
```
...
```
The tophits files are in `.exposure` format (as used in TwoSampleMR), column are labelled as such
```{r}
early_bmi_exp %>% head(4) %>%  glimpse()
```
...
The full summary data for exposures is in `.outcome` format (as used in TwoSampleMR), column are labelled as such. This is required for harmonisation at a later stage. 

```{r}
early_bmi_gwas %>% head(4) %>% glimpse()
```


### Create list objects for the exposures

Next, we will create two list objects:  tophits and full data of the exposures. These lists will be used as input for multivariable MR analysis in both TwoSampleMR and MVMR packages. If you have > 2 exposures to analyse, simply add the corresponding  `.exposure`
`/`.outcome` data into the lists. 
...
```
_If you want to use one local exposure and one from MR-Base, see the [section](#mixed_source) at end for details._

<br>

## Multivariable MR with TwoSampleMR package

Multivariable MR with TwoSampleMR is well-described in the original [guide][3], so here I'll be showing how to set up the first step of analysis (i.e. extracting instrumental variables from the exposures) if your exposure data is local. The guide only provides a working example for when your exposure traits of interest are available in MR-base (`mv_extract_exposures()` function).  _NB see the [<span style="color:red">note</span>](#note) later_

 I created a modified version of the original function `mv_extract_exposures()` - `get_mv_exposures()` which uses the tophits/full data lists that we generated earlier as the input and produces a data frame in the same format as the original `mv_extract_exposures()`. 
...
The function takes `tophits_list`, `exposure_dat`, `outcome.id.mrbase` (dataset id in MR-Base) OR `outcome.data` (full GWAS data frame of outcome trait, if using a local file), and returns `beta` and `se` values for each exposure in `mvmr_input$XGs` and same for the outcome in `mvmr_input$Y`.
...
```{r}
# the tidy up function is defined here
